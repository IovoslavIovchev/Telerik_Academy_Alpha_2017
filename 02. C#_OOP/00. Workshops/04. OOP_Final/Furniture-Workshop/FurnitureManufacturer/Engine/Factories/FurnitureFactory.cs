...
namespace FurnitureManufacturer.Engine.Factories
{
    public class FurnitureFactory : IFurnitureFactory
    {
        ...
        {
            return new Table(model, GetMaterialType(materialType.ToLower()).ToString(), price, height, length, width);
        }

        public IFurniture CreateChair(string model, string materialType, decimal price, decimal height, int numberOfLegs)
        {
            return new Chair(model, GetMaterialType(materialType.ToLower()).ToString(), price, height, numberOfLegs);
        }

        public IFurniture CreateAdjustableChair(string model, string materialType, decimal price, decimal height, int numberOfLegs)
        {
            return new AdjustableChair(model, GetMaterialType(materialType.ToLower()).ToString(), price, height, numberOfLegs);
        }

        public IFurniture CreateConvertibleChair(string model, string materialType, decimal price, decimal height, int numberOfLegs)
        {
            return new ConvertibleChair(model, GetMaterialType(materialType.ToLower()).ToString(), price, height, numberOfLegs);
        }
    }
}
